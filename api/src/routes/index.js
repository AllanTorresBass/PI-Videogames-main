	...
	if (!req.query.name) {
		...
		for (var i = 0; i < pages.length; i++) {
			...
			arr1.push(x); // push every obj
			  ...
			  console.log(req.query.name," line 56");
			arr1 = [];
...
router.post('/videogame', async (req, res) => {
	 let i=0;
	let { name, description, released, rating, genres, platforms, image } =	req.body; // destructuring the request's body expected params
	 
	// create a new object in db using body params
